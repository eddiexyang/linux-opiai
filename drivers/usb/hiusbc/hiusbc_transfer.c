					...
					unsigned int pcs, gfp_t gfp_flags)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_XFER_RING, "+\n");
...
{
	hiusbc_dbg(HIUSBC_DEBUG_XFER_RING,
		"ring = 0x%pK\n from dequeue = 0x%pK, num_trbs_free = %u\n",
	...
	hiusbc_dbg(HIUSBC_DEBUG_XFER_RING,
			"to dequeue = 0x%pK, num_trbs_free = %u\n",
...
static void xfer_ring_inc_enq(struct hiusbc_xfer_ring *ring)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_XFER_RING, "for ring = 0x%pK\n", ring);

	hiusbc_dbg(HIUSBC_DEBUG_XFER_RING,
		"From enqueue = 0x%pK, num_trbs_free = %u\n",
	...
	if (!chain) {
		hiusbc_dbg(HIUSBC_DEBUG_XFER_RING,
			"To enqueue = 0x%pK, num_trbs_free = %u\n",
			ring->enqueue, ring->num_trbs_free);
		return;
	}
...
static unsigned int calc_trbs_needed(struct hiusbc_req *hreq)
{
	...
	if (trbs_needed > 8)
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "!!!more than 8 TRBs in the TD!!!\n");
					...
					gfp_t gfp_flags)
{
	...
	while (!is_trbs_on_ring_enough(ring, trbs_needed)) {
		...
		hiusbc_dbg(HIUSBC_DEBUG_XFER_RING,
			"Enlarge Success, Xfer Ring = %pK, TRBs free = %u.\n",
				...
				u32 word0, u32 word1, u32 word2, u32 word3)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_XFER, "Queue a TRB to Ring = 0x%pK:\n", ring);
	hiusbc_dbg(HIUSBC_DEBUG_XFER, "TRB addr = 0x%pK\n", trb);
...
static void hiusbc_remove_request_from_ring(
	struct hiusbc_req *hreq, int status)
{
	struct hiusbc_ep *hep = hreq->hep;
	struct hiusbc_xfer_ring *ring = hep->ring;
	...
	struct hiusbc_req *hreq, int is_in)
{
	...
	if ((hreq->req.dma == DMA_ADDR_INVALID) || (hreq->epnum <= 1)) {
		...
		hiusbc_dbg(HIUSBC_DEBUG_ERR,
			"request 0x%pK: dma is not DMA_ADDR_INVALID\n",
			&hreq->req);
...
static void hiusbc_unmap_req_buf(struct hiusbc *hiusbc,
			struct hiusbc_req *hreq, int is_in)
{
	u8 dma_mapped = hreq->req.dma_mapped;
...
{
	struct hiusbc_xfer_ring *ring = hep->ring;
	unsigned int queued_length, req_length, trb_length, sent_length;
	bool first_trb = true;
	u64 buf_ptr;
	u32 ctrl;
	...
	req_length = hreq->req.length;

	if (hreq->req.num_sgs) {
		num_sgs = hreq->req.num_mapped_sgs;
		sg = hreq->req.sg;
		buf_ptr = (u64)sg_dma_address(sg);
		trb_length = sg_dma_len(sg);
	} else {
		buf_ptr = (u64)(hreq->req.dma);
		trb_length = req_length;
	}

	for (queued_length = 0; first_trb || queued_length < req_length;
					queued_length += sent_length) {
		if (hep->epnum == 0 || hep->epnum == 1)
		...
		if (queued_length + sent_length > req_length)
			sent_length = req_length - queued_length;
		...
		if (queued_length + sent_length < req_length) {
			...
			if (hreq->zlp) {
				hiusbc_dbg(HIUSBC_DEBUG_ERR,
					"zlp error %d\n",
					hep->epnum);
				hiusbc_queue_one_trb(ring,
					lower_32_bits(buf_ptr),
					upper_32_bits(buf_ptr),
					hiusbc_trb_xfer_length(sent_length),
					ctrl);

				buf_ptr += sent_length;
				trb_length -= sent_length;
				// This req is finished now, end this loop.
				queued_length += sent_length;
				// Queue a 0-packet TRB at last.
				sent_length = 0;

				/*
				 * last TD may stopped at a link TRB,
				 * we need to jump over it by calling hiusbc_prepare_xfer_ring.
				 */
				skip_link_trb(hep->ring);

				/* update cycle bit for 0-packet trb */
				ctrl &= ~HIUSBC_TRB_CYCLE;
				ctrl |= ring->pcs & HIUSBC_TRB_CYCLE;
			}
			/* Set this exactly before the last trb */
		...
		if (sg && trb_length == 0) {
			...
			// What do we do if sgs have been used up, but queued_length still not reach req_length?
			trb_length = sent_length;
	...
	hiusbc_dbg(HIUSBC_DEBUG_XFER,
		"Queued a TD on EP%u for usb request = 0x%pK, "
		"length = %u, first TRB = 0x%pK,"
		" last TRB = 0x%pK\n",
		hep->epnum,
		&hreq->req, hreq->req.length,
		hreq->first_trb,
		hreq->last_trb);
}
...
void hiusbc_finish_req(struct hiusbc_req *hreq, int status, bool giveback)
{
	struct hiusbc_ep *hep = hreq->hep;
	struct hiusbc *hiusbc = hep->hiusbc;
	u8 *temp_p_buf = NULL;
	...
	hiusbc_dbg(HIUSBC_DEBUG_XFER,
		"Giveback request 0x%pK on EP%u, actual = %u, status = %d, giveback = %d\n",
		&hreq->req, hreq->epnum, hreq->req.actual, hreq->req.status, giveback);
	...
	if (giveback) {
		if (hreq->req.complete != NULL) {
			spin_unlock(&hiusbc->lock);
			ktime_get_ts64(&hreq->giveback_time);
			hiusbc_dbg(HIUSBC_DEBUG_PERF, "request 0x%pK on EP%u, actual = %u, status = %d\n"
				"queued at %lld.%ld\n"
				"db at     %lld.%ld\n"
				"irq at    %lld.%ld\n"
				"event at  %lld.%ld\n"
				...
				hreq->queue_time.tv_sec, hreq->queue_time.tv_nsec,
				hreq->doorbell_time.tv_sec, hreq->doorbell_time.tv_nsec,
				hiusbc->irq_time.tv_sec, hiusbc->irq_time.tv_nsec,
				hiusbc->handle_event_time.tv_sec, hiusbc->handle_event_time.tv_nsec,
				hreq->giveback_time.tv_sec, hreq->giveback_time.tv_nsec);
...
void hiusbc_cleanup_exist_req_on_ep(
			struct hiusbc_ep *hep, int status, bool skip_last)
{
	struct hiusbc_xfer_ring *ring = hep->ring;
	struct hiusbc_req *hreq = NULL;
	struct hiusbc_req *n = NULL;
	struct hiusbc_req *last_hreq = NULL;
	bool giveback = true;

	hiusbc_dbg(HIUSBC_DEBUG_XFER, "+: EP = %u\n", hep->epnum);

	if (!list_empty(&hep->queue)) {
		last_hreq = list_last_entry(&hep->queue, struct hiusbc_req, queue);

		list_for_each_entry_safe(hreq, n, &hep->queue, queue) {
			if (skip_last && (hreq == last_hreq)) {
				giveback = false;
				skip_last = false;
			}

			hiusbc_finish_req(hreq, status, giveback);
		}
	}

	if (!list_empty(&ring->queue)) {
		giveback = true;
		last_hreq = list_last_entry(&ring->queue, struct hiusbc_req, queue);

		list_for_each_entry_safe(hreq, n, &ring->queue, queue) {
			if (usb_endpoint_xfer_control(hep->ep.desc))
				hiusbc_reclaim_invalid_trb_in_ctrl(ring);

			if (hreq->last_trb == NULL) {
				hiusbc_dbg(HIUSBC_DEBUG_ERR, "hreq last trb is null, hreq:%pK\n", hreq);
				return;
			}
			...
			const struct hiusbc_xfer_trb *trb, u32 level)
{
	...
	hiusbc_dbg(level, "Confirm TRB addr = 0x%pK:\n", trb);
...
int hiusbc_queue_req_bulk_int(
			struct hiusbc_ep *hep, bool new_req, gfp_t gfp_flags)
{
	struct hiusbc *hiusbc = hep->hiusbc;
	struct hiusbc_xfer_ring *ring = hep->ring;
	struct hiusbc_req *hreq = NULL;
	...
	if (hep->stalled || hep->set_halt_pending) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR,
			"EP%u is stalled, can't queue trbs to transfe ring.\n", hep->epnum);
		return 0;
	}
	...
	hiusbc_dbg(HIUSBC_DEBUG_XFER | HIUSBC_DEBUG_BULK,
	...
	hreq->doorbell_time = hiusbc->doorbell_time;

	if (ret < 0) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "Failed to send StartXfer on bulk_int EP%u! ret = %d\n",
			hep->epnum, ret);
		hiusbc_cleanup_exist_req_on_ep(hep, -ESHUTDOWN, new_req);
		return ret;
	}
			...
			struct hiusbc_ep *hep, bool new_req, gfp_t gfp_flags)
{
	...
	if (hep->xfer_state == HIUSBC_XFER_IDLE && !hep->nrdy_received) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "Can't queue, ep%u state is idle\n",
			hep->epnum);
		return 0;
	}
	...
	hreq->doorbell_time = hep->hiusbc->doorbell_time;

	if (ret < 0) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR,
			"Failed to send StartXfer cmd on isoc EP%u! ret = %d\n",
				hep->epnum, ret);
		hiusbc_cleanup_exist_req_on_ep(hep, -ESHUTDOWN, new_req);
		return 0;
	}
					...
					bool is_stall, gfp_t gfp_flags)
{
	...
	ktime_get_ts64(&hreq->queue_time);
	...
	if (ret) {
		hiusbc_cleanup_exist_req_on_ep(hep, -ECONNRESET, false);
		return ret;
	}
	...
	if ((setup->bRequest == USB_REQ_SET_ADDRESS) &&
		...
		(!is_stall)) {
		addr = (u8)(setup->wValue & 0x7f);
		hiusbc_dbg(HIUSBC_DEBUG_CTRL, "Set addr to 0x%x by ACK TRB\n", addr);
	...
	hiusbc_dbg(HIUSBC_DEBUG_CTRL,
		"Queued a %s TD on EP%u for request = %pK, "
		"first TRB = %pK, last TRB = %pK.\n",
		is_stall ? "STALL" : "ACK",
		hep->epnum,
		&hreq->req,
		hreq->first_trb,
	...
	if (ret < 0) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "Failed to send StartXfer on ctrl EP%u "
			"for status stage! ret = %d\n", hep->epnum, ret);
		hiusbc_cleanup_exist_req_on_ep(hep, -ESHUTDOWN, false);
		return ret;
	}
			...
			struct hiusbc_ep *hep, gfp_t gfp_flags)
{
	...
	if (ret < 0) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "Failed to send StartXfer cmd on ctrl EP%u "
...
exit:
	hiusbc_cleanup_exist_req_on_ep(hep, 0, false);
	return ret;
}
...
int hiusbc_ep_queue(struct usb_ep *ep, struct usb_request *req)
{
	...
	hreq->queue_time = start;
	hreq->doorbell_time = (struct timespec64){0, 0};
	hreq->event_time = (struct timespec64){0, 0};
	hreq->giveback_time = (struct timespec64){0, 0};
	...
	if (usb_endpoint_xfer_control(ep->desc)) {
		if ((hiusbc->cur_stage == HIUSBC_SETUP_STAGE) &&
			(hiusbc->next_stage == HIUSBC_DATA_STAGE) &&
			hiusbc->data_stage_dir) {
			hiusbc_dbg(HIUSBC_DEBUG_CTRL,
				"Move request = 0x%pK from EP0 to EP1",
				&hreq->req);
			hep = hiusbc->eps[1];
			hreq->hep = hep;
			hreq->epnum = hep->epnum;
	...
	hiusbc_dbg(HIUSBC_DEBUG_XFER,
		"Queued a request = 0x%pK for EP%u, length = %u, zero = %u, "
	...
	if (usb_endpoint_xfer_control(ep->desc)) {
		if (hiusbc->delayed_status == true) {
			if (hiusbc->next_stage != HIUSBC_STATUS_STAGE) {
				ret = 0;
				goto out;
			}
			hiusbc->delayed_status = false;
			usb_gadget_set_state(&hiusbc->gadget, USB_STATE_CONFIGURED);
			ret = hiusbc_queue_req_ctrl_status(hiusbc, 0, GFP_ATOMIC);
		} else {
		...
		ret = hiusbc_queue_req_isoc(hep, true, GFP_ATOMIC);
	} else {
		ret = hiusbc_queue_req_bulk_int(hep, true, GFP_ATOMIC);
	}

	pm_runtime_get(hiusbc->dev);

out:
	hiusbc_dbg(HIUSBC_DEBUG_XFER, "-: ret = %d\n", ret);
	return ret;
}
...
int hiusbc_ep_dequeue(struct usb_ep *ep, struct usb_request *req)
{
	...
	list_for_each_entry(hreq, &hep->queue, queue) {
		if (&hreq->req == req) {
			hiusbc_finish_req(hreq, -ECONNRESET, true);
			return 0;
		}
	}
	...
	if (hep->xfer_state != HIUSBC_XFER_END) {
		ret = hiusbc_send_cmd_end_transfer(hep, false);
		if ((ret < 0) && (!hep->end_xfer_pending))
			return ret;
	}

	hiusbc_dbg(HIUSBC_DEBUG_SYS, "Success endxfer cmplt\n");

	if (hreq->queued_to_ring) {
		hiusbc_dbg(HIUSBC_DEBUG_SYS, "Start giveback request\n");
		/*
