...
static const struct xhci_plat_priv_hisi xhci_plat_hisi = {
	.quirks = XHCI_RESET_ON_RESUME_HISI,
};

static int hiusbc_alloc_xhci_pdev(
			struct hiusbc *hiusbc,
			struct platform_device *hiusbc_pdev,
			struct platform_device **xhci)
{
	struct platform_device *xhci_temp = NULL;
	...
	host_res[1].end = irq;
	host_res[1].flags = IORESOURCE_IRQ;
	host_res[1].name = "hiusbc";

	xhci_temp = platform_device_alloc("xhci-hcd", PLATFORM_DEVID_AUTO);
	if (!xhci_temp)
		return -ENODEV;

	ret = platform_device_add_resources(
		xhci_temp, host_res, HIUSBC_HOST_RESOURCE_NUM);
	if (ret) {
		dev_err(hiusbc->dev, "Can't add resources to xhci.\n");
		platform_device_put(xhci_temp);
		return ret;
	}
...
int hiusbc_host_init(struct hiusbc *hiusbc)
{
	...
	ret = hiusbc_alloc_xhci_pdev(hiusbc, hiusbc_pdev, &xhci);
	if (ret)
		return ret;
	...
	if (!hiusbc->usb2_host_lpm_capable)
	...
	platform_device_add_data(xhci, &xhci_plat_hisi, sizeof(xhci_plat_hisi));
