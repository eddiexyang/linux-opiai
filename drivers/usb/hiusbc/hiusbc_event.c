...
{
	hiusbc_dbg(HIUSBC_DEBUG_EVT_RING, "+\n");
			...
			const struct hiusbc_evt_trb *event, int status)
{
	...
	list_for_each_entry_safe(hreq, n, &ring->queue, queue) {
		...
		if (hiusbc_xfer_trb_vrt_to_dma(
			...
			hiusbc_finish_req(hreq, status, true);
			break;
		} else {
	...
	/* for status req from function driver, include delay status case, we need giveback the req */
	if (!usb_endpoint_xfer_control(hep->ep.desc))
		return;
	hep = hep->hiusbc->eps[0];
					...
					const struct hiusbc_evt_trb *event)
{
	...
	case HIUSBC_CMPLT_CODE_ISO_OVERRUN:
		...
		hiusbc_dbg(HIUSBC_DEBUG_ERR,
			...
			"EP = %u.\n",
			hep->isoc_overrun_num, hep->epnum);
	...
	case HIUSBC_CMPLT_CODE_BABBLE_ERR:
		hep->babble_num++;
		hiusbc_dbg(HIUSBC_DEBUG_ERR,
			"%uth Xfer BABBLE Event, EP = %u.\n",
			hep->babble_num, hep->epnum);

		if (usb_endpoint_xfer_control(hep->ep.desc))
			hiusbc_dbg(HIUSBC_DEBUG_ERR, "This is a %s ctrl_babble event!\n",
				hiusbc->babble_expected ? "expected" : "unexpected");
					...
					int *status)
{
	...
	if (list_empty(&ring->queue) ||
			...
			hiusbc_xfer_trb_vrt_to_dma(
				ring->enqueue_seg, ring->enqueue) ==
					short_trb_addr) {
		hiusbc_dbg(HIUSBC_DEBUG_EVENT, "Can't find TRB on the ring! "
			"cmplt_code = %u, EP = %u.\n",
			cmplt_code, hep->epnum);
		return false;
	}
	...
	case HIUSBC_CMPLT_CODE_MISSED:
		...
		hiusbc_dbg(HIUSBC_DEBUG_ERR,
			...
			"EP = %u.\n",
			hep->miss_service_num, hep->epnum);
	...
	case HIUSBC_CMPLT_CODE_SHORT:
		hep->short_num++;
		hiusbc_dbg(HIUSBC_DEBUG_XFER,
			"%uth Xfer SHORT Event, EP = %u.\n",
			hep->short_num, hep->epnum);
		if (usb_endpoint_xfer_control(hep->ep.desc) &&
			...
			const struct hiusbc_evt_trb *event, int status)
{
	...
	list_for_each_entry_safe(hreq, n, &ring->queue, queue) {
		...
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "ERROR: this request lost cmplt event!\n");
	}
}
			...
			const struct hiusbc_evt_trb *event)
{
	...
	if (usb_endpoint_xfer_isoc(hep->ep.desc)) {
		...
		if (!list_empty(&hep->queue))
			hiusbc_queue_req_isoc(hep, false, GFP_ATOMIC);
	} else {
		if ((!list_empty(&hep->queue)) || hiusbc->db_on_received_event)
			hiusbc_queue_req_bulk_int(hep, false, GFP_ATOMIC);
	}
	...
	if ((hiusbc->next_stage != HIUSBC_SETUP_STAGE) ||
		...
		if (hiusbc->eps[0]->xfer_state != HIUSBC_XFER_END) {
			ret = hiusbc_send_cmd_end_transfer(
				hiusbc->eps[0], false);
			if (ret)
				hiusbc_dbg(HIUSBC_DEBUG_ERR, "Failed to send EndXfer to EP0\n");
		}
		...
		if (hiusbc->eps[1]->xfer_state != HIUSBC_XFER_END) {
			ret = hiusbc_send_cmd_end_transfer(
				hiusbc->eps[1], false);
			if (ret)
				hiusbc_dbg(HIUSBC_DEBUG_ERR, "Failed to send EndXfer to EP1\n");
		}

		hiusbc_cleanup_exist_req_on_ep(hiusbc->eps[0], -ECONNRESET, false);
		hiusbc_cleanup_exist_req_on_ep(hiusbc->eps[1], -ECONNRESET, false);
	}
}
			...
			struct hiusbc *hiusbc, struct hiusbc_ep *hep)
{
	...
	if (list_empty(&ring->queue) &&
		...
		ret = hiusbc_send_cmd_start_transfer(hep, start_mf,
				...
				hep->ring->pcs, 1, 0);
		if (ret < 0)
			hiusbc_dbg(HIUSBC_DEBUG_ERR, "Failed to send StartXfer cmd "
				...
				struct hiusbc_ep *hep, u32 cmd_type)
{
	...
	case HIUSBC_CMD_TYPE_END_TRANSFER:
		hiusbc_dbg(HIUSBC_DEBUG_EVENT, "Cmd Type: END_TRANSFER.\n");
		handle_end_xfer_success_event(hiusbc, hep);

		break;
	case HIUSBC_CMD_TYPE_DISABLE_EP:
		hiusbc_dbg(HIUSBC_DEBUG_EVENT, "Cmd Type: DISABLE_EP.\n");
...
static void hiusbc_dev_event_disconnect(struct hiusbc *hiusbc)
{
	...
	reg = hiusbc_readl(hiusbc->dev_regs,
		DEV_CTL_OFFSET);
	reg &= ~(ENABLE_U1_INITIATE_MASK |
		ENABLE_U2_INITIATE_MASK);
...
static void set_bulk_u2_lpm_mode(struct hiusbc *hiusbc)
{
	u32 reg;
	reg = hiusbc_readl(hiusbc->dev_regs,
		TRPU_DEV_USB_CTRL_OFFSET);
	if (hiusbc->dis_gadget_u2_lpm_bulk_always_idle) {
		reg |= TRPU_DEV_U2_LP_MOD_MASK;
	} else {
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "bulk not always idle\n");
		reg &= ~TRPU_DEV_U2_LP_MOD_MASK;
	}
	hiusbc_writel(reg, hiusbc->dev_regs,
		TRPU_DEV_USB_CTRL_OFFSET);
...
static void config_lpm(struct hiusbc *hiusbc, enum usb_device_speed speed)
{
	...
	/* borrow usb3_dev_lpm_capable flag for test */
	if (((speed == USB_SPEED_HIGH) || (speed == USB_SPEED_FULL)) &&
		hiusbc->usb2_dev_lpm_capable) {
		hiusbc_dbg(HIUSBC_DEBUG_EVENT, "enable USB2 LPM!\n");
		set_bulk_u2_lpm_mode(hiusbc);

		reg = hiusbc_readl(hiusbc->dev_regs,
			DEV_CTL_OFFSET);
		reg &= ~BESL_THRESHOLD_SUSPEND_MASK;
		...
		hiusbc_dbg(HIUSBC_DEBUG_EVENT, "disable USB2 LPM!\n");
		reg = hiusbc_readl(hiusbc->dev_regs,
			DEV_CFG_OFFSET);
		reg &= ~LPM_ENABLE_MASK;
		hiusbc_writel(reg, hiusbc->dev_regs,
...
static void hiusbc_dev_event_connect(struct hiusbc *hiusbc)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_EVENT, "old_speed = %d\n", old_speed);
	...
	if ((old_speed == speed) &&
		...
		hiusbc->last_is_connect_event = true;
		return;
	}
	...
	*/
	...
	hiusbc_dbg(HIUSBC_DEBUG_EVENT, "Connected at speed = %d\n", speed);
...
static int hiusbc_wakeup_active_bulk_eps(struct hiusbc *hiusbc)
{
	struct hiusbc_ep *hep = NULL;
	u8 epnum;
	int ret = 0;
...
static void hiusbc_dev_event_suspend(struct hiusbc *hiusbc)
{
	...
	if (hiusbc->disconnect_in_suspend) {
		hiusbc_dbg(HIUSBC_DEBUG_EVENT, "call disconnect in suspend event!!!\n");
...
static int hiusbc_handle_event(struct hiusbc_evt_ring *event_ring)
{
	...
	ktime_get_ts64(&hiusbc->handle_event_time);
	...
	hiusbc_dbg(HIUSBC_DEBUG_EVT_RING,
		"Handle event(TRB) = 0x%pK, EP = %u, cmplt_code = %u.\n",
...
out:
	...
	hiusbc->handle_event_time = (struct timespec64){0, 0};
...
{
	struct hiusbc *hiusbc = (struct hiusbc *)(uintptr_t) param;
	struct hiusbc_evt_ring *event_ring = hiusbc->event_ring;
	...
	hiusbc_dbg(HIUSBC_DEBUG_EVT_RING, "+\n");
	if (hiusbc->events_pending == 0)
		goto out;

	dequeue_backup = event_ring->dequeue;

	hiusbc_dbg(HIUSBC_DEBUG_EVT_RING,
		"Start to handle interrupt at Deq = %pK.\n",
			event_ring->dequeue);

	while (hiusbc_handle_event(event_ring) > 0) {
		event_total++;
		event_counter++;
		if (event_counter >= event_ring->size / 4) {
			/* update erdq */
			dequeue = hiusbc_evt_trb_vrt_to_dma(event_ring, event_ring->dequeue);
			if (!dequeue)
				hiusbc_dbg(HIUSBC_DEBUG_ERR, "1/4 Bad sw event ring dequeue ptr!\n");

			dequeue &= HIUSBC_DEQUEUE_EBDP0_LO_MASK;
	...
	hiusbc->events_pending = false;

	hiusbc_dbg(HIUSBC_DEBUG_EVT_RING,
		"Handled %d evt, from:%pK, to:%pK\n",
			event_total, dequeue_backup,
			event_ring->dequeue);

	if (event_total == 0)
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "Interrupt comes with empty event ring!\n");

	dequeue = hiusbc_evt_trb_vrt_to_dma(event_ring, event_ring->dequeue);
	if (!dequeue)
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "Bad sw event ring dequeue ptr!\n");

	dequeue &= HIUSBC_DEQUEUE_EBDP0_LO_MASK;
	dequeue |= DEV_EVENT_HANDLER_BUSY_0_MASK;
	reg = lower_32_bits(dequeue);
	hiusbc_writel(reg, hiusbc->dev_regs,
		hiusbc_dev_ebdp0_offset(event_ring->int_idx));
	reg = upper_32_bits(dequeue);
...
out:
	...
	enable_irq(hiusbc->irq);
	hiusbc_dbg(HIUSBC_DEBUG_EVENT | HIUSBC_DEBUG_SYS |HIUSBC_DEBUG_EVT_RING,
			"-: handled %d evt\n", event_total);
}
...
irqreturn_t hiusbc_interrupt(int irq, void *_hiusbc)
{
	...
	ktime_get_ts64(&hiusbc->irq_time);

	spin_lock_irqsave(&hiusbc->lock, flags);

	hiusbc_dbg(HIUSBC_DEBUG_EVT_RING, "+ %pK\n", hiusbc);

	if (hiusbc->events_pending) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "irq handler busy!\n");
		ret = IRQ_HANDLED;
		goto error;
	}
	...
	reg = hiusbc_readl(hiusbc->dev_regs,
		DEV_STATUS_OFFSET);
	if (!(reg & DEV_EVENT_INTERRUPT_MASK)) {
		dev_err(hiusbc->dev,
			"Interrupt comes but dev_status_IP is 0, "
			"dev_status = 0x%x!\n", reg);
		goto error;
	}
	reg |= DEV_EVENT_INTERRUPT_MASK;
	hiusbc_writel(reg, hiusbc->dev_regs,
		DEV_STATUS_OFFSET);
	...
	if (!(reg & DEV_INT_PENDING_0_MASK)) {
		dev_err(hiusbc->dev,
			"Interrupt comes but IMAN_IP is 0, IMAN = 0x%x!\n",
			reg);
		goto error;
	}
	reg |= DEV_INT_PENDING_0_MASK;
	hiusbc_writel(reg, hiusbc->dev_regs,
		hiusbc_dev_iman_offset(event_ring->int_idx));

	hiusbc->events_pending = true;

	disable_irq_nosync(irq);
	tasklet_schedule(&hiusbc->bh);
	ret = IRQ_HANDLED;
