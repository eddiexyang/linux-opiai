...
#define USB_HEADSET_LOW_RESOLUTION_MAX_ESIT 192
#define USB_HEADSET_MEDIUM_RESOLUTION_MAX_ESIT 384
#define USB_HEADSET_HIGH_RESOLUTION_MAX_ESIT 576
#define USB_HEADSET_LOW_RESOLUTION_PAD 175
#define USB_HEADSET_MEDIUM_RESOLUTION_PAD 171
#define USB_HEADSET_HIGH_RESOLUTION_PAD 167
#define USB_FS_INT_IN_MAX_ESIT_PAYLOAD 350
#define USB_FS_INT_IN_MAX_PACKET 64
#define USB_LS_INT_IN_MAX_ESIT_PAYLOAD 9
#define USB_LS_INT_IN_MAX_PACKET 8
#endif
...
#define hiusbc_get_cmd_done_info(x) \
	(((x) & DEV_CMD_DONE_INFO_MASK) >> \
	DEV_CMD_DONE_INFO_SHIFT)

#define hiusbc_ctl_besl_suspend(x) \
	(((x) << BESL_THRESHOLD_SUSPEND_SHIFT) & \
	BESL_THRESHOLD_SUSPEND_MASK)
...
#define hiusbc_get_u2portsc_speed(x) \
	(((x) & DEV_U2_PORT_SPEED_MASK) >> \
	DEV_U2_PORT_SPEED_SHIFT)
...
#define hiusbc_drd_mode(x) \
	(((x) << DRD_WORK_MODE_SHIFT) & \
	DRD_WORK_MODE_MASK)
#define hiusbc_get_drd_mode(x) \
	(((x) & DRD_WORK_MODE_MASK) >> \
	DRD_WORK_MODE_SHIFT)
...
struct hiusbc {
	...
	u32 usb_id;
	u32 u2_only;
	void __iomem *io_subctrl_regs;
	void __iomem *pcs_regs;

	int gpio_count;
	u32 *gpio_host_value;
	u32 *gpio_dev_value;
	u32 *gpio_suspend_value;
	unsigned *gpios;
};
