...
static void hiusbc_enable_events(struct hiusbc *hiusbc)
{
	...
	reg = DISCONNECT_EVENT_EN_MASK |
		CONNECT_EVENT_EN_MASK |
		USB_RESET_EVENT_EN_MASK |
		SUSPEND_ENTRY_EVENT_EN_MASK |
		U3_RESUME_EVENT_EN_MASK |
		L1_ENTRY_EVENT_EN_MASK |
		L1_RESUME_EVENT_EN_MASK |
		HIUSBC_DEVT_EN_CMD_SET_HALT |
		VENDOR_TEST_EVENT_EN_MASK;
...
void hiusbc_set_ux_exit(struct hiusbc *hiusbc)
{
	...
	if (!ux_exit_interval) {
		hiusbc_writel(0, hiusbc->dev_regs,
			DEV_USB3_U1EL_OFFSET);
		hiusbc_writel(0, hiusbc->dev_regs,
			DEV_USB3_U2EL_OFFSET);
	} else {
		...
		reg = hiusbc_readl(hiusbc->dev_regs,
			DEV_U3_UX_EXIT_CFG_OFFSET);
		reg &= ~DEV_UX_EXIT_INTERVAL_MASK;
		reg |= ((ux_exit_interval
			<< DEV_UX_EXIT_INTERVAL_SHIFT) &
				DEV_UX_EXIT_INTERVAL_MASK);
		hiusbc_writel(reg, hiusbc->dev_regs,
			DEV_U3_UX_EXIT_CFG_OFFSET);
		...
		reg = (((u32)hiusbc->u1_pel <<
			DEV_USB3_U1PEL_SHIFT) |
			hiusbc->u1_sel);
		hiusbc_writel(reg, hiusbc->dev_regs,
			DEV_USB3_U1EL_OFFSET);

		reg = (((u32)hiusbc->u2_pel <<
			DEV_USB3_U2PEL_SHIFT) |
			hiusbc->u2_sel);
		hiusbc_writel(reg, hiusbc->dev_regs,
			DEV_USB3_U2EL_OFFSET);
	}
			...
			const struct hiusbc *hiusbc)
{
	...
	u2_portsc = hiusbc_readl(hiusbc->dev_regs,
		DEV_U2_PORTSC_OFFSET);
	u3_portsc = hiusbc_readl(hiusbc->dev_regs,
		DEV_U3_PORTSC_OFFSET);

	hiusbc_dbg(HIUSBC_DEBUG_SYS,
		"Confirm u2_portsc = 0x%x, u3_portsc = 0x%x\n",
		u2_portsc, u3_portsc);

	u2_ccs = !!(u2_portsc & DEV_U2_PORT_SPEED_MASK);
	u3_ccs = !!(u3_portsc & DEV_U3_PORT_SPEED_MASK);
		...
		enum usb_device_speed speed, enum hiusbc_link_state state)
{
	...
	case USB_SPEED_HIGH:
		portsc = hiusbc_readl(hiusbc->dev_regs,
		DEV_U2_PORTSC_OFFSET);
		portsc &= ~DEV_U2_PLS_MASK;
		portsc |= DEV_U2_LWS_MASK |
	...
	case USB_SPEED_SUPER_PLUS:
		portsc = hiusbc_readl(hiusbc->dev_regs,
		DEV_U3_PORTSC_OFFSET);
		portsc &= ~DEV_U3_PLS_MASK;
		portsc |= DEV_U3_LWS_MASK |
					...
					u16 imodi)
{
	...
	reg &= ~DEV_INT_MOD_INTERVAL_0_MASK;
		...
		const struct hiusbc_cmd *parm)
{
	...
	hiusbc_writel(parm->word0, hiusbc->dev_regs,
		DEV_CMD_WORD0_OFFSET);
	hiusbc_writel(parm->word1, hiusbc->dev_regs,
		DEV_CMD_WORD1_OFFSET);
	hiusbc_writel(parm->word2, hiusbc->dev_regs,
		DEV_CMD_WORD2_OFFSET);
	hiusbc_writel(parm->word3, hiusbc->dev_regs,
		DEV_CMD_WORD3_OFFSET);
	hiusbc_writel(DEV_CMD_VLD_MASK,
		hiusbc->dev_regs, DEV_CMD_CTRL_OFFSET);

	do {
		reg = hiusbc_readl(hiusbc->dev_regs,
			DEV_CMD_CTRL_OFFSET);
		if (reg & DEV_CMD_DONE_MASK) {
			switch (hiusbc_get_cmd_done_info(reg)) {
			case HIUSBC_CMD_FINISH:
				ret = 0;
				break;
			case HIUSBC_CMD_PARM_ERROR:
				hiusbc_dbg(HIUSBC_DEBUG_ERR, "Cmd done info: Params Err.\n");
				ret = -EINVAL;
				break;
			case HIUSBC_CMD_ABORT:
				hiusbc_dbg(HIUSBC_DEBUG_ERR, "Cmd done info: Aborted.\n");
				ret = -EAGAIN;
				break;
			case HIUSBC_CMD_FAIL:
				hiusbc_dbg(HIUSBC_DEBUG_ERR, "Cmd done info: Failed.\n");
				ret = -EAGAIN;
				break;
			case HIUSBC_CMD_FRM_EXPIRED:
				hiusbc_dbg(HIUSBC_DEBUG_ERR, "Cmd done info: FrameID expired.\n");
				ret = -EINVAL;
				break;
			case HIUSBC_CMD_EVT_LOSS:
				hiusbc_dbg(HIUSBC_DEBUG_ERR, "Cmd done info: Event Loss.\n");
				ret = -EINVAL;
				break;
			default:
				hiusbc_dbg(HIUSBC_DEBUG_ERR, "Unknown: done_info = 0x%x\n",
	...
	if (timeout == 0) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "Cmd timeout!\n");
		ret = -ETIMEDOUT;
	}
...
static int hiusbc_send_cmd_enable_ep(struct hiusbc_ep *hep)
{
	...
	if (usb_endpoint_xfer_int(ep_desc) ||
		...
		if (interval) {
			...
			hiusbc_dbg(HIUSBC_DEBUG_ERR, "bInterval for EP%u is 0? should be 1~16.\n",
				hep->epnum);
			hep->si = 1;
		}
	}
	...
	u32 start_mf, dma_addr_t buffer_ptr, u8 dcs, u8 init, u8 ctrl_num)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_XFER | HIUSBC_DEBUG_CMD,
	...
	ktime_get_ts64(&hiusbc->doorbell_time);
					...
					bool wait_event)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_XFER, "Queuing a EndXfer cmd, EPNum = %u.\n", hep->epnum);
	...
	if (!ret) {
		...
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "Failed to send EndXfer to EP%u\n", hep->epnum);
		return ret;
	}
	...
	if (time_remain == 0) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "Wait endxfer cmplt event timeout!\n");
		ret = -ETIMEDOUT;
	} else if (time_remain == -ERESTARTSYS) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "Wait endxfer cmplt event was interrupted!\n");
		ret = -ETIMEDOUT;
	}
			...
			const struct usb_endpoint_descriptor *desc)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_EP, "  EPNum = %u\n", hep->epnum);
...
static int hiusbc_gadget_ep_disable(struct usb_ep *ep)
{
	...
	if (hep->disable_pending) {
		...
		if (time_remain == 0 || time_remain == -ERESTARTSYS) {
			hiusbc_dbg(HIUSBC_DEBUG_ERR,
				"Wait disable ep cmplt event err %lu!\n",
				time_remain);
			ret = -ETIMEDOUT;
		}
		hiusbc_dbg(HIUSBC_DEBUG_CMD,
				...
				struct usb_ep *ep, gfp_t gfp_flags)
{
	...
	if (!hreq) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "alloc hreq fail!");
		return NULL;
	}
			...
			struct usb_ep *ep, struct usb_request *req)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_XFER, "Free a usb request = 0x%pK, for EP%u.\n",
		req, hep->epnum);
...
static int hiusbc_handle_clear_ep_stall(struct hiusbc_ep *hep)
{
	...
	if (!ret) {
		...
		if (!list_empty(&hep->queue)) {
			hiusbc_queue_req_bulk_int(hep, false, GFP_ATOMIC);
		} else {
			...
			hiusbc_dbg(HIUSBC_DEBUG_XFER | HIUSBC_DEBUG_BULK,
				"Restart ep%u after clear stall. init = %u\n",
				hep->epnum, init);
			...
			if (ret < 0) {
				hiusbc_dbg(HIUSBC_DEBUG_ERR,
					"Failed to send StartXfer on bulk_int EP%u! ret = %d\n",
					hep->epnum, ret);
				hiusbc_cleanup_exist_req_on_ep(hep, -ESHUTDOWN, false);
				return ret;
			}
...
{
	hiusbc_dbg(HIUSBC_DEBUG_EP, "+: set = %d\n", set);

	if (usb_endpoint_xfer_isoc(hep->ep.desc)) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "Can't %s Halt to Isoc EP%u.\n",
...
static int hiusbc_gadget_ep_set_halt(struct usb_ep *ep, int value)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_XFER | HIUSBC_DEBUG_EP, "+\n");
...
error:
	spin_unlock_irqrestore(&hiusbc->lock, flags);
	hiusbc_dbg(HIUSBC_DEBUG_SYS, "-: ret = %d\n", ret);
	return ret;
}
		...
		int is_selfpowered)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_SYS, "is_selfpowered = %d", is_selfpowered);
...
int hiusbc_run_stop(struct hiusbc *hiusbc, int is_on)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_SYS,
		...
		!!(reg & DEV_RUN_STOP_MASK));

	if (is_on && !(reg & DEV_RUN_STOP_MASK)) {
	...
	if (hiusbc->ltssm_dump)
		reg_dfx |= CSR_DFX_STATE_WR_EN_MASK;
	else
		reg_dfx &= ~CSR_DFX_STATE_WR_EN_MASK;
...
int hiusbc_restart(struct hiusbc *hiusbc)
{
	...
	hiusbc->last_is_connect_event = false;
				...
				const struct usb_ctrlrequest *setup)
{
	...
	if ((hiusbc->running == 0) ||
		(hiusbc->gadget_driver == NULL) ||
		(hiusbc->gadget_driver->setup  == NULL))
		return -ENOENT;
...
void hiusbc_gadget_reset(struct hiusbc *hiusbc)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_EVENT, "+ gadget.speed = %u\n", hiusbc->gadget.speed);
...
static int hiusbc_gadget_init_interrupt(struct hiusbc *hiusbc)
{
	...
	/* alloc a event ring, int_idx is 0. */
...
STATIC int hiusbc_init_eps(struct hiusbc *hiusbc)
{
	...
	for (epnum = 0; epnum < HIUSBC_EP_NUMS; epnum++) {
		...
		hiusbc_dbg(HIUSBC_DEBUG_SYS,
			"Successful init EP%u, Xfe Ring = 0x%pK, "
			"start TRB = 0x%pK, "
			"num_segs = %u, num_trbs = %u.\n",
			epnum, hep->ring,
			hep->ring->enqueue,
			hep->ring->num_segs, hep->ring->num_trbs_free);
		hiusbc_dbg(HIUSBC_DEBUG_SYS, "  ep = %pK\n", &hep->ep);
	}
...
STATIC int hiusbc_mem_init(struct hiusbc *hiusbc)
{
	...
	(void)memset_s(&hiusbc->gadget, sizeof(hiusbc->gadget),
					0x00, sizeof(hiusbc->gadget));
	hiusbc->gadget.ops = &hiusbc_gadget_ops;
	hiusbc->gadget.speed = USB_SPEED_UNKNOWN;
	hiusbc->gadget.sg_supported = hiusbc->sg_supported;
	hiusbc->gadget.lpm_capable = hiusbc->usb2_dev_lpm_capable;
...
static void hiusbc_gadget_release(struct device *dev)
{
	struct usb_gadget *gadget = container_of(dev, struct usb_gadget, dev);
	(void)memset_s(&gadget->dev, sizeof(gadget->dev), 0x00, sizeof(gadget->dev));
}
...
int hiusbc_gadget_init(struct hiusbc *hiusbc)
{
	...
	usb_initialize_gadget(hiusbc->dev, &hiusbc->gadget, hiusbc_gadget_release);

	hiusbc->gadget.dev.platform_data = hiusbc;

	hiusbc->bh.func = hiusbc_intr_handle_evt_ring;
	hiusbc->bh.data = (unsigned long)(uintptr_t)hiusbc;
	...
	ret = usb_add_gadget(&hiusbc->gadget);
	if (ret) {
...
exit:
	hiusbc_dbg(HIUSBC_DEBUG_ERR, "%s-: ret = %d", __func__, ret);
	return ret;
}
...
void hiusbc_gadget_exit(struct hiusbc *hiusbc)
{
	...
	usb_del_gadget(&hiusbc->gadget);

	hiusbc_stop(hiusbc);

	free_irq(hiusbc->irq, hiusbc);
	tasklet_kill(&hiusbc->bh);
