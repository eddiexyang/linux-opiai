/*
 * hiusbc_debugfs_common.c -- Driver for Hisilicon USB Controller.
 *
...
{
	hiusbc_dbg(HIUSBC_DEBUG_TEMP, "ep%u is %s\n",
		hep->epnum, hep->enabled ? "enabled" : "disabled");
	hiusbc_dbg(HIUSBC_DEBUG_TEMP, "event counter on ep%u:\n"
			"success: %u\n"
			"short: %u\n"
			"nrdy: %u\n"
			"babble: %u\n"
...
void hiusbc_clear_dev_event_counter(struct hiusbc *hiusbc)
{
	if (memset_s(&hiusbc->stat, sizeof(hiusbc->stat),
		...
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "%s memset err!\n", __func__);
}
...
void hiusbc_show_dev_event_counter(struct hiusbc *hiusbc)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_TEMP, "dev event counter:\n"
			"disconnect: %u\n"
			"connect: %u\n"
			"rst: %u\n"
			"plc: %u\n"
...
int hiusbc_debug_knock_ep(struct hiusbc_ep *hep)
{
	...
	if (ret) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "Failed to send StartXfer cmd on EP%u! ret = %d\n",
			hep->epnum, ret);
		return ret;
	}
...
void hiusbc_cptest_set_to_cp0(struct hiusbc *hiusbc)
{
	...
	reg = hiusbc_readl(hiusbc->u3_link_regs,
		CSR_CP_PAT_SEL_OFFSET);
	reg |= CSR_CP_PAT_SEL_MASK;
...
void hiusbc_cptest_next_pattern(struct hiusbc *hiusbc)
{
	...
	reg = hiusbc_readl(hiusbc->u3_link_regs,
		CSR_CP_PAT_SW_CTRL_OFFSET);
	reg |= CSR_CP_PAT_SW_CTRL_MASK;
	hiusbc_writel(reg, hiusbc->u3_link_regs,
		CSR_CP_PAT_SW_CTRL_OFFSET);

	reg = hiusbc_readl(hiusbc->u3_link_regs,
		CSR_CP_PAT_SW_OFFSET);
	reg |= CSR_CP_PAT_SW_MASK;
	hiusbc_writel(reg, hiusbc->u3_link_regs,
		CSR_CP_PAT_SW_OFFSET);
