/*
 * hiusbc_setup.c -- Device Request Related for Hisilicon USB Controller.
 *
...
static void hiusbc_req_get_status_cmplt(struct usb_ep *ep, struct usb_request *req)
{
}
				...
				const struct usb_ctrlrequest *setup)
{
	...
	if (setup->wValue == 0x01) {
		dev_err(hiusbc->dev, "Not support PTM in device mode.\n");
		return -EINVAL;
	}
	...
	*buf = cpu_to_le16(status);
	...
	hiusbc->dev_req.req.complete = hiusbc_req_get_status_cmplt;
		...
		const struct usb_ctrlrequest *setup, bool set)
{
	...
	if (!hiusbc->usb3_dev_lpm_capable) {
		...
		hiusbc_dbg(HIUSBC_DEBUG_CTRL, "usb3_dev_lpm_capable is off\n");
		return 0;
	}
		...
		const struct usb_ctrlrequest *setup, bool set)
{
	 ...
	 */
	if (epnum <= 1 && set) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "Host can't set halt to ctrl EP%u.\n", epnum);
		return -EINVAL;
	}
				...
				const struct usb_ctrlrequest *setup)
{
	...
	if (state != USB_STATE_CONFIGURED) {
		dev_err(hiusbc->dev,
			"will set addr to 0x%x in Status Stage.\n", addr);
		return 0;
	}
...
static void hiusbc_enable_accept_lpm(struct hiusbc *hiusbc)
{
	...
	if ((hiusbc->gadget.speed >= USB_SPEED_SUPER) &&
		...
		if (hiusbc->usb3_dev_lpm_u1_accept)
			reg |= ENABLE_U1_ACCEPT_MASK;

		if (hiusbc->usb3_dev_lpm_u2_accept)
			reg |= ENABLE_U2_ACCEPT_MASK;

		hiusbc_writel(reg, hiusbc->dev_regs,
			DEV_CTL_OFFSET);
		hiusbc_dbg(HIUSBC_DEBUG_CTRL, "Enable %s %s Accept.\n",
			hiusbc->usb3_dev_lpm_u1_accept ? "U1" : " ",
			hiusbc->usb3_dev_lpm_u2_accept ? "U2" : " ");
	}
}
		...
		const struct usb_ctrlrequest *setup)
{
	...
	reg = hiusbc_readl(hiusbc->dev_regs,
		DEV_U3_ISOCH_DELAY_OFFSET);
	reg &= ~DEV_ISOCH_DELAY_MASK;
	reg |= (setup->wValue & DEV_ISOCH_DELAY_MASK);
	hiusbc_writel(reg, hiusbc->dev_regs,
		DEV_U3_ISOCH_DELAY_OFFSET);
