...
int hiusbc_gpio_request(struct hiusbc *hiusbc)
{
...
int hiusbc_gpio_alloc(struct hiusbc *hiusbc)
{
...
int hiusbc_gpio_config(struct hiusbc *hiusbc)
{
	...
	hiusbc->gpio_count = of_gpio_named_count(np, "gpios");
	if (hiusbc->gpio_count <= 0) {
...
int hiusbc_write_allow_cfg_reg(const struct qos_allow_config_type *cfg, u32 reg_addr)
{
	u32 reg;
	void __iomem *io_subctrl;
...
int hiusbc_get_rw_allow_cfg(struct qos_allow_config_type *cfg, u32 reg_addr)
{
	u32 reg;
	void __iomem *io_subctrl;
...
int hiusbc_get_rw_allow_mode(struct qos_allow_config_type *cfg, u32 reg_addr)
{
	u32 reg;
	void __iomem *io_subctrl;
...
int hiusbc_write_otsd_cfg_reg(const struct qos_otsd_config_type *cfg, u32 reg_addr)
{
	u32 reg;
	void __iomem *io_subctrl;
	io_subctrl = ioremap(reg_addr, IO_SUBCTL_REG_SIZE);
...
int hiusbc_get_rw_otsd_mode(struct qos_otsd_config_type *cfg, u32 reg_addr)
{
...
int hiusbc_axi_qos_init(struct hiusbc *hiusbc)
{
	int ret = 0;
